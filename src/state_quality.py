...
def state_quality_realistic(resources: dict) -> float:
    ...
    food_factor = -30 if resources['Food'] == 0 else max(math.log(resources['Food'] / resources['Population']) * 8, 30)
    housing_factor = -20 if resources['Housing'] == 0 else max(math.log(resources['Housing'] / resources['Population'], 3) * 7,
    ...
    potentialfossilenergy_waste_factor = (resources['PotentialFossilEnergyWaste'] / wastespread) * -8
    potentialrenewableenergy_waste_factor = (resources['PotentialRenewableEnergyWaste'] / wastespread) * -6

    #overpopulation factor
    overpopulation_factor = 0 if resources['Population'] / resources['AvailableLand'] <= 5 else (resources['Population'] / resources['AvailableLand']) * -2
